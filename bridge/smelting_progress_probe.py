"""Deterministic smelting progress probe for Dwarf Fortress.

This module queries the DF runtime for the number of active metal smelting jobs (type "df.job_type.SmeltMetal") and returns a JSON‑serialisable dictionary with a single key "smelting_jobs".  The implementation follows the pattern used by other probes in the repository and does not affect existing public interfaces.
"""
...
def _lua_smelting_snapshot() -> str:
    ...
    whose type is ``df.job_type.SmeltMetal``.  The result is printed as JSON so
    ...
    if df.global and df.global.world and df.global.world.jobs then
        for _, job in ipairs(df.global.world.jobs.list) do
            if job.job_type == df.job_type.SmeltMetal then
                count = count + 1;
            end
        end
    end
