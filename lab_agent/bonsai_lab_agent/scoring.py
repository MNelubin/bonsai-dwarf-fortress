...
CALIBRATION = {
    ...
    # bank the whole horizon's digging at round 0, then top up only when the standing
    # backlog runs low. The fresh reference rose 0.5971 -> 0.6187 and on the mature
    # save the plain digger with the bank (158 tiles) now leads. The evolved player's
    # normalised score fell from 1.54/1.23 to 1.32/0.99 as a result -- which is the
    # cycle working: what the player finds becomes the rule, the yardstick rises, and
    # what is left is genuinely beyond the rules.
    ...
    # 2026-09-14, evening: raw fish stopped counting as food (a Fishery has to prepare
    # it) and the teacher learned the fish chain. The idle floor fell by exactly the
    # fisherdwarf's catch, 0.467857 -> 0.457143; v3 0.617422 (k=3, one episode at 0.588
    # with two buildings instead of four - the plots' placement is the fresh fort's
    # run-to-run noise now).
    ("ourfort16-final", 3600): {"noop": 0.457143, "ref": 0.617422},
    ...
    # - run-to-run). The mature fort has a refuse pile and forbidden stock, so its
    ...
    # touch.
    ("region3-lab", 3600):     {"noop": 0.530528, "ref": 0.611531},   # re-measured twice on 2026-09-14 (reach fix, then raw fish): 0.6112-0.6115
