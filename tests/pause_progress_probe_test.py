"""Deterministic public test for bridge.pause_progress_probe.probe_pause_jobs.

This test imports the pause progress probe, calls the function with the default timeout,
and verifies that the returned value is either a dictionary with a single key "pause_jobs"
and an integer count, or None when the probe cannot communicate with the DF runtime.
...
class PauseProgressProbePublicTest(unittest.TestCase):
    """Basic sanity‑check that the pause progress probe returns the expected contract."""
    def test_probe_returns_schema_or_none(self) -> None:
        ...
        if result is None:
            ...
            return
        # Expect {'pause_jobs': <int>}
        self.assertIsInstance(result, dict)
        self.assertIn('pause_jobs', result)
        count = result['pause_jobs']
